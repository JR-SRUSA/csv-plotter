(() => {
  ...
  // Hooks into the optional vehicle-sim.js module (Simulate Vehicle panel). No-ops until
  // window.VehicleSim.init() swaps in the real ones below (right after sessionsFiles exists),
  // so a build without the module (scripts/build-production.js --no-vehicle-sim) still works.
  const simHooks = {
    applyLinkedSimChannels: () => Promise.resolve(),
    persistSyntheticLogForSession: () => Promise.resolve(),
  ...
  function addProcessedLog(file, processed, rawInput) {
    ...
    applySavedImporterConfigToLog(file.name, newLog.id).then(() => simHooks.applyLinkedSimChannels(newLog.id));
  ...
  function appendSessionRow(el, log) {
    ...
    addToSessionBtn.addEventListener('click', () => {
      ...
      const ready = (log.meta && log.meta.synthetic)
        ? simHooks.persistSyntheticLogForSession(log)
        : Promise.resolve();
  ...
  // Sim channels computed for a logged lap (see vehicle-sim.js) are kept as a companion
  // stored file, "<source> - sim channels.csv", that is not a log in its own right. These
  // stay in app.js rather than the sim module because every "load a stored file" path and
  // backup restore must keep skipping such records even in a build without the sim.
  const SIM_CHANNELS_FILE_SUFFIX = ' - sim channels.csv';

  function simChannelsFileName(sourceName) {
    return String(sourceName) + SIM_CHANNELS_FILE_SUFFIX;
  }
  ...
  function importVehiclesRidersFromFile(file) {
    ...
    file.text().then((text) => {
      ...
      Promise.all([
        ...
        simHooks.refreshVehicleSimLists();
  ...
  // Uploaded files are persisted in IndexedDB so they survive page reloads. They're loaded
  // into the plotter on demand (Load All Stored Files / Pick Uploaded Data), and
  // automatically only right after a backup restore that reloaded the page
  // (loadFilesPendingAfterRestore).
  //
  ...
  // Everything the Simulate Vehicle module (vehicle-sim.js) reads from app.js. Must stay in
  // sync with that file's HOST_KEYS -- its init() names anything missing in the console.
  if (window.VehicleSim) {
    Object.assign(simHooks, window.VehicleSim.init({
      // State
  ...
  // onlyNames (optional Set): load just those stored files instead of all of them.
  // Resolves to the number of files handed to parseFile.
  function loadStoredFilesIntoPlotter(onlyNames) {
    return getAllFilesFromDB().then((entries) => {
      const alreadyLoaded = new Set(logs.map(l => l.name));
      let count = 0;
      entries.forEach((entry) => {
        if (onlyNames && !onlyNames.has(entry.name)) return;
  ...
  // Backup restore across a reload (see restoreFromBackup's applyBackup): the list of files
  // it stored but deliberately didn't parse before reloading.
  const LOAD_AFTER_RESTORE_KEY = 'csvPlotterLoadAfterRestore';

  function canUseSessionStorage() {
    try {
      sessionStorage.setItem('csvPlotterStorageProbe', '1');
      sessionStorage.removeItem('csvPlotterStorageProbe');
      return true;
    } catch {
      return false;
    }
  ...
  // Called once at startup. Read-and-remove, so a later manual refresh doesn't load them again.
  function loadFilesPendingAfterRestore() {
    let names = null;
    try {
      const raw = sessionStorage.getItem(LOAD_AFTER_RESTORE_KEY);
      sessionStorage.removeItem(LOAD_AFTER_RESTORE_KEY);
      names = raw ? JSON.parse(raw) : null;
    } catch {}
    if (!Array.isArray(names) || !names.length) return;
    loadStoredFilesIntoPlotter(new Set(names)).then((count) => {
      setStoredFilesStatus(`Restored backup: loaded ${count} file(s).`);
    });
  }
  ...
  function restoreFromBackup(file) {
    ...
    function applyBackup(parsed, fileEntries) {
      ...
      // Restored settings are only read at startup, so a backup carrying any of them ends in
      // a page reload -- which would throw away anything parsed here first. In that case the
      // files are only stored now, and loaded once after the reload instead (see
      // loadFilesPendingAfterRestore); sessionStorage carries the list across it.
      const restoredNames = fileEntries.filter((e) => e.name && typeof e.text === 'string').map((e) => e.name);
      const willReload = !!(parsed && parsed.settings && typeof parsed.settings === 'object')
        && SETTINGS_STORAGE_KEYS.some((key) => key in parsed.settings);
      const deferParse = willReload && canUseSessionStorage();
      const storePromises = fileEntries.map((entry) => {
        ...
        return storeFileInDB(entry.name, entry.text).then(() => {
          if (deferParse) return;
          const alreadyLoaded = new Set(logs.map(l => l.name));
          if (!alreadyLoaded.has(entry.name) && !String(entry.name).endsWith(SIM_CHANNELS_FILE_SUFFIX)) {
            const blob = new Blob([entry.text], { type: 'text/plain' });
      ...
      Promise.all(storePromises).then(() => {
        ...
        if (parsed && parsed.settings && typeof parsed.settings === 'object') {
          ...
          if (applied > 0) {
            if (deferParse) {
              try { sessionStorage.setItem(LOAD_AFTER_RESTORE_KEY, JSON.stringify(restoredNames)); } catch {}
            }
        ...
        // Settings were present but none applied (e.g. a malformed theme value), so no reload
        // after all -- load the files that were only stored above.
        if (deferParse) loadStoredFilesIntoPlotter(new Set(restoredNames));
  ...
  // Last, once everything above is wired up: finish a backup restore that reloaded the page.
  loadFilesPendingAfterRestore();
